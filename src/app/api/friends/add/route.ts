...
export async function POST(req: Request) {
	try {
		...
		// check if user have a friend request from this person (if yes, don't send it)
		...
		if (isAlreadyReceiveFriendRequest) {
			return new Response(
				"This user has already invited you. Please check your friend requests.",
				{ status: 400 }
			);
		}
		...
		if (isAlreadyFriends) {
			return new Response("You're already friends with this user.", {
				status: 400,
			});
		}
