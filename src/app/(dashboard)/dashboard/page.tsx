...
const page = async ({}) => {
	...
	const [friends, groups] = await Promise.all([
		...
		getUsersGroups(session.user.id),
	]);
	...
	return (
		<div className="container py-12">
			...
			{friendsWithLastMessage.length + groupsWithLastMessage.length ===
			0 ? (
				<p className="text-sm text-zinc-500">Nothing to show here...</p>
			) : (
				[...friendsWithLastMessage, ...groupsWithLastMessage]
					.sort(
						(i, j) =>
							i.lastMessage.timestamp - j.lastMessage.timestamp
					)
					.map((friendOrGroup) => (
						<div
							key={
							...
							>
								...
								<div>
									<h4 className="text-lg font-semibold">
										{"name" in friendOrGroup
											? friendOrGroup.name
											: friendOrGroup.group}
									</h4>
									<p className="mt-1 max-w-md">
										<span className="text-zinc-400">
											{friendOrGroup.lastMessage
