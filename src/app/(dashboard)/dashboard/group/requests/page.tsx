...
const page = async () => {
	...
	// ids of people who sent current user a group requests
	let incomingGroupRequests: IncomingGroupRequest[] = [];
	// get all incoming group requests
	if (incomingSenderIds && incomingSenderIds.flat().length >= 1)
		incomingGroupRequests = await Promise.all(
