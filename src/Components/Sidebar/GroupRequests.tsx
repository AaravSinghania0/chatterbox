...
const GroupRequests: FC<GroupRequestsProps> = ({
	...
	useEffect(() => {
		...
		// console.log("listening to ", `user:${sessionId}:incoming_group_requests`)
		...
		const groupRequestHandler = ({
			...
			setGroupRequests((prev) => [
				...prev,
				{ senderId, senderEmail, groupName },
			]);
		};
		...
		<>
			{groupRequests.length === 0 ? (
				<p className="text-sm text-zinc-500">
					You don't have any group requests...
				</p>
			) : (
				groupRequests.map((request) => (
					...
					>
						<UserPlus className="text-slate-900" />
						...
						<button
							onClick={() =>
								acceptGroup(request.senderId, request.groupName)
							}
						...
						<button
							onClick={() =>
								denyGroup(request.senderId, request.groupName)
							}
